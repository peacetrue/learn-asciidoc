...
image::draw/elf.mmd.svg[width=75%]
...
使用 `image` 标签将文本绘图工具生成的图片引入到文档中，例如：`image::draw/elf.mmd.svg[width=75%]`。
...
#获取 draw-test.adoc
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/peacetrue/learn-asciidoc/master/docs/antora/modules/ROOT/pages/draw-test.adoc)"
#不使用绘图插件，无法生成图片
...
#生成文档，提示在 xxx 目录下找不到 asciidoctor-kroki
...
#在 xxx 目录下，安装 asciidoctor-kroki 插件
